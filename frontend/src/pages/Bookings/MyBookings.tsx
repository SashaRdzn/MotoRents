        ...
        {(data ?? []).map((b) => (
          <div key={b.id} className={styles.card}>
            <div className={styles.row}>
              <div className={styles.name}>#{b.id} · {b.motorcycle?.brand ?? ''} {b.motorcycle?.model ?? ''}</div>
              <div className={styles.status}>{b.status}</div>
            </div>
            <div className={styles.row}>
              <div>Период: {new Date(b.rental_period.start_time).toLocaleString()} — {new Date(b.rental_period.end_time).toLocaleString()}</div>
